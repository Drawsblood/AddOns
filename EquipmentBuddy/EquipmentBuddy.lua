...
local addon = ...
	if addon == "EquipmentBuddy" then
		if not ebname and not ebname2 and not ebpvpname and not ebpvpname2 then
			...
			if ebinstance == true then
				myCheckButton_GlobalName7:SetChecked(true)
			elseif ebinstance == nil then
				myCheckButton_GlobalName7:SetChecked(true)
			else
				myCheckButton_GlobalName7:SetChecked(false)
			end
			if ebcombat == true then
				myCheckButton_GlobalName8:SetChecked(true)
			elseif ebcombat == nil then
				myCheckButton_GlobalName8:SetChecked(true)
			else
...
elseif event == "TRADE_SKILL_SHOW" then -- for cooking set
	...
	if not IsTradeSkillLinked() and GetTradeSkillLine() == "Cooking" and not InCombatLockdown() then
