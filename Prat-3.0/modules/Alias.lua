...
L:AddLocale("koKR",  
{
	...
	["dealiasing command /%s to /%s"] = "/%s 가 /%s 로 별칭 해제됩니다.",
	["deleting alias \"/%s\" (previously aliased as \"/%s\")"] = "별칭 \"%s\"를 삭제합니다. (\"/%s\"로 설정되어 있었음)",
	["Display extra information in the chat frame when commands are dealiased"] = "명령어가 별칭으로 수행될 때, 채팅창에 추가정보를 표시합니다.",
	...
	inline = "자동바꿈",
	["<keyword> - finds all aliases matching <keyword> (cmd aliases: /findalias)"] = "<키워드> - <키워드>가 포함된 모든 별칭을 찾습니다. (명령어: /findalias)",
	listaliases = "별칭목록",
	["list all aliases"] = "모든 별칭을 보여줍니다.",
	[" - list all aliases; supply <keyword> to search for matching aliases (cmd aliases: /listallaliases)"] = " - 모든 별칭 목록; 별칭을 찾기 위한 <키워드> 지원 (명령어: /listallaliases)",
	["matching aliases found: %d"] = "일치된 별칭 찾음: %d 개",
	module_desc = "UNIX의 alias 명령과 비슷한 방법으로 슬래쉬 명령어를 대체하는데 사용 하는 /alias 명령을 추가합니다.",
	module_name = "별칭",
	...
	Options = "옵션",
	["Options for altering the behaviour of Alias"] = "별칭 수행에 대한 옵션",
