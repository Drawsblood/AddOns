...
local function ShowArrow(x, y, text)
...
local TreasureMapsIds = {
    [1116] = true, -- Draenor
    [1152] = true, -- Garrison Level 1 (H)
    [1330] = true, -- Garrison Level 2 (H)
    [1153] = true, -- Garrison Level 3 (H)
    [1158] = true, -- Garrison Level 1 (A)
    [1331] = true, -- Garrison Level 2 (A)
    [1159] = true, -- Garrison Level 3 (A)
...
local function FilterMinimapPOI(v, x, y, mapID, radius)
    local showTreasures = DRTSOptions.enabled and TreasureMapsIds[mapID]
...
local function onUpdate(self,elapsed)
    ...
    if not TreasureMapsIds[currentmapid] then return end
    ...
    if interval >= 1 then
        ...
        if DRTSOptions.debug then
            ShowArrow(closest.x,closest.y, format("%s (%u)\n%.02f yd", closest.name, closest.id, dist))
        else
            ShowArrow(closest.x,closest.y, format("%s\n%.02f yd", closest.name, dist))
        end
    end
end
...
local function onEvent(self, event, ...)
    if event == "ADDON_LOADED" then
        ...
        MinimapPOIHandler:SetDataSource(DRTS.treasures)
...
local function TreasureMap_CreatePOI(index)
    local button = CreateFrame("Button", "TreasureMapFramePOI"..index, WorldMapButton)
...
local function DrawTreasurePOI()
    ...
    local showTreasures = (not isContinent) and (worldMapID > 0) and DRTSOptions.enabled and (TreasureMapsIds[currentmapid] or TreasureMapsIds[mapID])
    ...
    for k,v in pairs(DRTS.treasures) do
        ...
        if showTreasures and (DRTSOptions.debug or (not DRTSOptions.hidden[v.id] and trCheck and tmCheck and choise1Check and choise2Check)) then
            ...
            if onMap then
                ...
                local description = v.desc ~= "" and v.desc or format("Draenor Treasure: %s!", TreasureQuestsDesc[c1quest] or TreasureQuestsDesc[tmquest] or "Unknown")

                x = x * WorldMapButton:GetWidth()
                y = -y * WorldMapButton:GetHeight()
                treasureMapPOI:SetPoint("CENTER", "WorldMapButton", "TOPLEFT", x, y)
